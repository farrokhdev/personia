...
import { Banner, Logo, Name, Slogan } from './header'
...
export function PlatformBox(props: Props): ReactElement {
  return (
    <Box>
      {props.loading ? (
        <Link to={'#'}>
        ...
        <Link to={'/platforms/get/' + props.platform.id}>
          <div className={'row-1'}>
            <Banner platform={props.platform} height={'144'} />
            <div className={'flex'}>
              <Logo platform={props.platform} />
              <div>
                <Name platform={props.platform} />
                <Slogan platform={props.platform} />
              </div>
            </div>
          </div>
          <div className={'row-2'}>
            <p dangerouslySetInnerHTML={{__html: decodeURIComponent(props.platform.projectMission)}}></p>
          </div>
        </Link>
      )}
    </Box>
