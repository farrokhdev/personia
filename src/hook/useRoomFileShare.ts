...
import { PeerActions,  FileOfferMetadata, Peer } from '../models'
import { PeerRoom, PeerHookType, fileTransfer } from '../services'
...
const isInlineMediaFile = (file: File) => {
  return ['image', 'audio', 'video', 'application'].includes(file.type.split('/')[0])
}
...
export function useRoomFileShare({
  ...
  const handleFileShareStart = async (files: any) => {
    //const inlineMediaFiles = [...files].filter(isInlineMediaFile)
    const inlineMediaFiles = files
